...
module.exports = class Kick extends Command {
  ...
  static match (message) {
    return message.content.startsWith(config.prefix+"kick")
  }
      ...
      if(Number.isInteger(parseInt(victim))){
        ...
        if(message.guild.members.get(victim) != null){
          ...
          if(Permission.hasHigherRole(message.member, victim) || Permission.isOwner(message.member)){
            ...
            Report.kick(user, message.author, reason, bot)
            message.guild.members.get(victim).kick(reason)
            Success.kick(user.tag, victim, reason, message)
        ...
        Error.kickError(message.channel)
